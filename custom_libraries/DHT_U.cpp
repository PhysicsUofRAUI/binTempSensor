#include "catch.hpp"
#include "bin_sensor_library/DHT_U.hpp"
...
TEST_CASE("Test positive values", "[DHT-U]") {
    ...
    REQUIRE(new_event.temperature == (float)0.7);
    
    sensor.temp = 1.8;
    
    sensor.temperature().getEvent(&new_event);
    
    REQUIRE(new_event.temperature == (float)1.8);
    
    sensor.temp = 5.4;
    
    sensor.temperature().getEvent(&new_event);
    
    REQUIRE(new_event.temperature == (float)5.4);
    
    sensor.temp = 100.5;
    
    sensor.temperature().getEvent(&new_event);
    
    REQUIRE(new_event.temperature == (float)100.5);
}
...
TEST_CASE("Test special values", "[DHT-U]") {
  ...
  REQUIRE(new_event.temperature == (float)0.0);
